...
void CLogger::SetEditControl(CEdit* pEdit)
{
    m_pLogEdit = pEdit;
}
...
void CLogger::WriteToEdit(const CString& logLine)
{
    if (m_pLogEdit && ::IsWindow(m_pLogEdit->GetSafeHwnd()))
    {
        int len = m_pLogEdit->GetWindowTextLength();
        m_pLogEdit->SetSel(len, len);
        m_pLogEdit->ReplaceSel(logLine);
    }
}
